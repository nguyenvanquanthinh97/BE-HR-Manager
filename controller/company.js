...
module.exports.getStatistics = async (req, res, next) => {
	...
	try {
		...
		res
			.status(200)
			.json({ message: 'Get Statistics Success', numOfOffices, numOfDepartures, numOfProjects, numOfUsers });
	} catch (error) {
		next(error);
	}
};
...
module.exports.editOKR = async (req, res, next) => {
     ...
     */
	if (taskIds.length > 0) {
		taskIds = taskIds.map(({ projectId, taskId }) => ({
	...
	const schema = Joi.object().keys({
		...
		updatedOKRsProgress: Joi.array().items(
			Joi.object().keys({
...
module.exports.getCompanyInfo = async (req, res, next) => {
	...
	try {
		...
		res
			.status(200)
			.json({
				message: 'Get Company Info Success For OKRS',
				companyInfos,
				officeInfos,
				departureInfos,
				userInfos
